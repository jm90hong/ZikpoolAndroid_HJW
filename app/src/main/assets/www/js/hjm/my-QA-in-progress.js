function readyMyQAinProgress() {
  ...
  $(document).on('click','.practice-zikpool-my-btn',function(){
    ...
    //todo 위 3개의 값으로 zikpoolconfirm 호출 후 직풀연습 시작.
    zikpoolConfirm({
            title:'실시간 과외 연습',
    ...
    if(payment_state=='r'){
        ...
        if(zcClass.length > 0){
            ...
            if(zc_payment_state=='r'){
                //todo 직풀이 환불인 경우.
                zikpoolConfirm({
                    title:'직풀(실시간과외) 포인트 환불',
                    content:'해당 직풀채팅에 대한 선생님 신고가 처리되었습니다.<br/>'+obj.q_point+' 직풀포인트를 환불 합니다.',
                    confirm:function(){
                ...
                zikpoolConfirm({
                    title:'직풀채팅 완료하기',
                    content:'직풀채팅(질문포함) 을 완료하시겠습니까? <br/>(프로필에서 확인 가능)',
                    confirm:function(){
                        var obj = {
                            member_job:'s',
  ...
  $(document).on('click','.complete-answer-my-btn',function(){
    ...
    if(zcClass.length > 0){
        ...
        if(zc_report_state !='y'){
            $t='직풀채팅 완료하기';
            $c='직풀채팅(답변포함) 을 완료하시겠습니까?<br/>(프로필에서 확인 가능)';
        }else{
            $t='직풀채팅 완료하기(신고)';
            $c='직풀채팅(답변포함) 을 완료하시겠습니까?<br/>(프로필에서 확인 가능)';
        }
...
//todo 학생이 직풀 채팅을 완료하였을때 콜백 함수.(Activity 에서 호출)
function callbackWhenStudentCompleteZikpoolChat($question_idx,$teacher_idx,$answer_idx){
    ...
    var msg ='<div class="zc-msg-zikpool-chat-complete-flag">완료된 직풀 채팅입니다.</div>';
...
function removeCompletedQuestion($question_idx){
    ...
    qClass.fadeOut(200,function(){
        ...
        //todo 직풀 채팅 존재 여부 확인 후 같이 삭제. 해당 질문에 직풀채팅이 존재하면 직풀채팅도 삭제함.
...
function removeCompletedAnswer($answer_idx){
    ...
    aClass.fadeOut(200,function(){
        ...
        //todo 안드로이드 Room에서 질문 사진 삭제.
        window.android_header.deleteQuestionImageInRoom(question_idx);
        //todo 직풀 채팅 존재 여부 확인 후 같이 삭제. 해당 답변과 관련된 직풀채팅이 존재하면 삭제.
...
function checkZikpoolChatAndDeleteAll($type,$idx){
    ...
    if(zcClass.length >0){
        ...
        if(otherZcClass.length == 0){
            //todo 삭제하려고 하는 회원이미지를 사용하는 직풀 채팅이 없음. -> 안드로이드 Room 에서 회원이미지 삭제.
