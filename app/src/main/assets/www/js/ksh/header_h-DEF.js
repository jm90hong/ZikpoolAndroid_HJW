...
//initGuideHeader();
    // swiper_API를 이용하여 swiper 구현
    mainswiper = new Swiper('.swiper-mainTab',{
      resistance:true,
      resistanceRatio:0,
      on:{
        init:function(){
          $('.main-one-tab').on('click',function(){
            var idx = $(this).data('index');
            mainswiper.slideTo(idx,200,function(){});
          });
        },
        slideChange:function(){
          ...
          if(tab4.isRealtime){
            if(idx==3){
                $('.main-one-tab').css('color','#999999');
                $('.is-tab4-realtime[data-type="on"]').css('color','#307afa');
                $('.swiper-scrollbar-drag').css('background','#307afa');
            }else{
                $('.main-one-tab').css('color','#999999');
                $('.is-tab4-realtime[data-type="on"]').css('color','#8AB2F9');
                $('.main-one-tab[data-index='+idx+']').css('color','#000000');
          ...
          //todo 최초에 한번 직풀 선생님에 진입 할때 리스트 호출.
          if(tab4.isFirstEnter && idx==3){
    ...
    $('#left_menu_btn').click(function() {
        ...
        if(ZP_SESSION.login == 'on'){
          showBackForSwiper();
          $('#left_menu_screen').show( "slide", 100,function(){
            mainswiper_left_login.update();
          });
        }else{
          showBackForSwiper();
          $('#left_menu_screen_LN').show( "slide", 100,function(){
    ...
    $('.left_menu_close').on('click',function() {
        ...
        if(ZP_SESSION.login == 'on'){
          $('#left_menu_screen').stop().hide( "slide", 100,function(){
              //mainswiper.update();
              mainswiper_left_login.slideTo(0,10, function(){});
          });
        }else{
          $('#left_menu_screen_LN').stop().hide( "slide", 100,function(){
              //mainswiper.update();
              mainswiper_left_logout.slideTo(0,10, function(){});
          });
        }
        hideBackForSwiper();
   ...
   $('#go-to-support').click(function(){
    ...
    $('#go-to-add-question-in-m-s').on('click',function(){
      $('.left_menu_close').trigger('click');
      $('#go-to-add-question').trigger('click');
    });
