...
var userid='나'; //유저 닉네임
var roomid='pratice509'; //chat_code
var authority='t'; //직업 t or s
    ...
    roomSystemSetting();
      ...
      iconPenWidth.on('click',function handler(){
        if(iconPenWidth.hasClass('zik-clicked')){
          ...
          iconPenWidth.find('img').attr('src','img/room/penwidth-b.png');
      ...
      iconEraseAll.on('click',function(){
         zikpoolConfirm({
             ...
             confirm:function(){
                 removeCanvasAll(whichCanvasUsed,'self'); //practice 라서 self
             }
         })
       });
...
function roomSystemSetting(){
  ...
  $('#exit-room-btn').on('click',function(){
    zikpoolConfirm({
        title:'과외 연습 종료',
        content:'과외 연습을 종료하시겠습니까?',
        confirm:function(){
            android_practicezikpool.leaveZikpoolRoom();
        }
    })
  })
};
...
function getAndSetQuestionImagesFromServer($qi){
      return new Promise(function(resolve, reject) {
          $.ajax({
           url : super_url+'getQuestionImg?q_idx='+$qi,
           type : "get",
           dataType : "json",
...
function getAndSetAnswerImagesFromServer($ai){
      return new Promise(function(resolve, reject) {
          $.ajax({
           url : super_url+'getAnswerImg?a_idx='+$ai,
           type : "get",
           dataType : "json",
           success : function(data) {
