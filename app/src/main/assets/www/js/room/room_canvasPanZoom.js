...
//  var elem = document.getElementById('question-canvas-board-wrapper');
//     zm = new Zoom(elem, {
//        rotate: false
//    });
//});
...
function Zoom(elem, config, wnd) {
    ...
    var handleTouchEvent = function(cb) {
        //console.log('handleTouchEvent is called : '+canvasMode);
        if(true){
            return function(evt) {
                if(evt.cancelable){
                    evt.preventDefault();
    ...
    var handleZoom = handleTouchEvent(function(touches){
      //console.log('handleZoom is called : '+canvasMode);
      if(canvasMode=='panzoom'){
        var numOfFingers = touches.length;
         //console.log('Log [0] :'+numOfFingers+' / ');
        if(numOfFingers>1){
          //투터치로 확대 할 때 다시 그리기.
          // var w = questionCanvasBoard.getBoundingClientRect().width;
          // var h = questionCanvasBoard.getBoundingClientRect().height;
          // questionCanvasBoard.width=w;
          // questionCanvasBoard.height=h;
        }

        if(numOfFingers != me.curTouch){
            //console.log('Log [1] :'+numOfFingers+' / ');
            ...
            //console.log('Log [2] :'+numOfFingers+' / ');
            setDest(touches);
            me.previewZoom();
        }
      }
    ...
    var handleTouchStart = handleTouchEvent(function(touches) {
        //console.log('handleTouchStart is called : '+canvasMode);
        if(canvasMode=='panzoom'){
          if (touches.length === 1) {
...
Zoom.prototype.repaint = function() {
    ...
    //console.log('elem : '+this.resultantZoom.css());
