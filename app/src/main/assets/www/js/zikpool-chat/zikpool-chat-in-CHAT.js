...
function init(){
    ...
    if(ThisChatObj.report_state=='n' && ThisChatObj.payment_state =='n'){
        ...
        $("textarea#msg-input-edit-box").on('click',function(){
            window.android_zikpoolchat.zikpoolToast('신고/완료 된 수잘친채팅에서 채팅이용은 불가능합니다.');
        });
    }
...
function setUI(){
    ...
    $('.zc-btn').on('click',function(){
        ...
        if(type=='detail-question'){
            ...
            if(ThisChatObj.payment_state=='n'){
                ...
                window.android_zikpoolchat.zikpoolToast('완료/환불된 수잘친채팅의 선생님은 신고할 수 없습니다.');
            }
        }else if(type=='complete-zikpool'){
            if(ThisChatObj.payment_state !='n' || ThisChatObj.report_state !='n'){
                window.android_zikpoolchat.zikpoolToast('신고된 수잘친채팅은 완료 할 수 없습니다.');
            }else{
                window.android_zikpoolchat.popupWindow('on');
                $('.zc-popup-window[data-type="'+type+'"]').show();
            }
        }else{
            //todo start-zikpool(수잘친시작) , pause(일시정지)
            if(ThisChatObj.report_state=='n'){
                if(ThisChatObj.payment_state == 'n'){
                    window.android_zikpoolchat.popupWindow('on');
                    $('.zc-popup-window[data-type="'+type+'"]').show();
                ...
                window.android_zikpoolchat.zikpoolToast('신고된 수잘친채팅에서는 이용 할 수 없습니다.');
            }

        }
    });

    //todo 팝업 창에서 최종 버튼 누를때. > 수잘친 시작하기, 일시정지 하기, 일시정지 해제하기, 수잘친 완료하기
    $('.zc-final-btn').on('click',function(){
        ...
        if(type=='pause'){
            ...
            //todo 수잘친 과외방 시작. ZikpoolRoomActivity호출
            ...
            window.android_zikpoolchat.zikpoolToast('수잘친채팅에 다시 입장해주세요.');
        }
    });


    //todo 수잘친 시작 메세지 박스 에서 버튼 클릭 이벤트[학생에게 제공되는 버튼]
    $(document).on('click', '.sz-final-btn-for-student', function(){
        ...
        //todo  수잘친 참가하기
            if(ThisChatObj.my_job=='s'){
                if(ThisChatObj.payment_state =='n'){
                    //todo [STEP 1] - chatidx의 room 값 알아오고 콜백으로 수잘친 시작.
...
let onFinalBtn = {
    ...
    if(zc_check.reply){
        ...
        $('#loading-text-1').css('color','#fff').html('수잘친채팅(실시간 과외) 완료 처리중...');
        $('#upload-loading-wall').show();
        $.ajax({
            ...
            success:function(msg){
                 ...
                 ThisChatObj.payment_state='y';
                //todo [STEP 2] 수잘친 방에서 완료 효과 주기 html.
                $('.sz-btn-space').html(myHtml.completed);

                disableZikpoolChat();
                //todo [STEP 3] 나의 수잘친 채팅 리스트 완료 표시 업데이트. -> HeaderActivity 로 전달.
...
function untieChatObj(zc){
    ...
    //todo 최신 수잘친 참가하기 버튼을 제외하고 나머지는 전부 무효처리.
...
function changeTextInLoadingWindow($type){
    if($type=='pause'){
        ...
        $('#loading-text-1').css('color','var(--cr-main)').html('수잘친채팅이 완료 되었습니다.');
    }
}
